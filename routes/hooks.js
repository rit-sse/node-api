...
router
  .route('/dockerbuilds')
    .post((req, res) => {
      ...
      client.publish('dockerbuilds', 'build for ' + req.body.repository.name + ' has finished',
        (err, response) => {
        ...
        request.post({ url: req.body.callback_url, json: responseData })
...
router
  .route('/:channel')
    .post((req, res) => {
      // Publish the bots message to dockerbuilds 
      client.publish(req.params.channel, JSON.stringify(req.body),
        (err, response) => {
          res.send('ok');
        });  
    });
